...
eqs <- function(time, y, pars) {
  ...
  for(j in 1:A){
    ...
    ba<- pars$gi/(pars$bw-pars$aw*ma)*(pars$bw-pars$aw*ma)/(sqrt((pars$bw-pars$aw*ma)^2+s[1:A]))*exp(-(pars$Temp- ma)^2/(2*(pars$bw-pars$aw*ma)^2+s[1:A])) - pars$ki
    bp<- pars$gi/(pars$bw-pars$aw*mp)*(pars$bw-pars$aw*mp)/(sqrt((pars$bw-pars$aw*mp)^2+s[(A+1):(A+P)]))*exp(-(pars$Temp- mp)^2/(2*(pars$bw-pars$aw*mp)^2+s[(A+1):(A+P)])) - pars$ki
    
    bar_ba<- pars$gi/(pars$bw-pars$aw*ma)*exp(-(pars$Temp- ma)^2/(2*(pars$bw-pars$aw*ma)^2+s[1:A]))*s[1:A]*(pars$bw-pars$aw*ma)*(pars$Temp - ma)/((pars$bw-pars$aw*ma)^2 + s[1:A])^1.5
    bar_bp<-pars$gi/(pars$bw-pars$aw*mp)*exp(-(pars$Temp- mp)^2/(2*(pars$bw-pars$aw*mp)^2+s[(A+1):(A+P)]))*s[(A+1):(A+P)]*(pars$bw-pars$aw*mp)*(pars$Temp - mp)/((pars$bw-pars$aw*mp)^2 + s[(A+1):(A+P)])^1.5 
    ...
    for(k in 1:P){
      ...
      dudt_A<- pars$h2[1:A]*(bar_ba+ bij%*%np) #mean trait dynamics
      dudt_P<- pars$h2[(A+1):(P+A)]*(bar_bp+ bji%*%na) #mean trait dynamics
